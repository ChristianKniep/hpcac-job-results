...
#SBATCH --workdir=/chome/cluser/job_results/mpi/venus/omp-sys/2014-11-06_19-02-50
#SBATCH --job-name=mpi-bench_omp-sys
#SBATCH --partition=venus
...
for JOB_BIN in osu_alltoall;do
    ...
    echo "mpirun --mca btl self,openib  ${BIN_PATH}/${JOB_BIN} > ${JOB_PATH}/${JOB_BIN}.out"
    mpirun --mca btl "self,openib"  ${BIN_PATH}/${JOB_BIN} > ${JOB_PATH}/${JOB_BIN}.out
    EC=$?
